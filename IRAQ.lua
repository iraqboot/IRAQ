...
RAMBOv2_SUDO = sudos.KpOwner   -------bot owner
RAMBOv2_TOKEN = sudos.Token -----RAMBO_TOKEN
bot_id = sudos.Bot_ID ---id bot
RAMBOv2 = tonumber(_redis.Bot_ID)

function Run()
print('\27[93m>Developer:\27[39m'..' '..'@BECIQ')
end
------------------------boT ID   BY RAMBO ----------------------
print(string.sub(_redis.Bot_ID,1,0))
local bot_id = redis:get(RAMBOv2.."Bot:KpBotAccount") or tonumber(_redis.Bot_ID)
...
redis:sadd(RAMBOv2.."Bot:KpSudos", user)
end
redis:setex(RAMBOv2.."bot:reload", 7230, true)
redis:set(RAMBOv2.."Bot:KpOwnerBot", config.KpOwner or 0)
...
local OwnerKP_ = redis:get(RAMBOv2.."Bot:KpOwnerBot")
local text = "List of Authorities :\n"
local user_info_ = redis:get(RAMBOv2.."user:Name" .. OwnerKP_)
...
redis:del(RAMBOv2.."Bot:KpSudos")
redis:del(RAMBOv2.."Bot:KpOwnerBot")
...
local KPdata = redis:get(RAMBOv2.."Botid" .. bot_id) or "\n"
local BotKPdata = redis:get(RAMBOv2.."KpOwnerBot" .. config.KpOwner) or "\n"
...
for v, user in pairs(config.Sudo_Users) do
local SudoData = redis:get(RAMBOv2.."KpSudos" .. user)
if SudoData then
...
local lists = redis:smembers(RAMBOv2.."Bot:KpSudos")
for k, v in pairs(lists) do
setnumbergp_three(v)
end
redis:setex(RAMBOv2.."bot:reload", 7230, true)
...
local Kp_Owner = redis:get(RAMBOv2.."Bot:KpOwnerBot")
...
print("\27[0;35m>>"..[[
تم كتابه السورس بواسطه @STOOFY
WRITING THE SOURCE BY : @STOOFY
CH SOURCE : @BECIQ
]].."\n\027[00m")
-----------------------function is_KP----------by RAMBO------------------------
...
local var = false
if redis:sismember(RAMBOv2.."Bot:KpSudos", msg.sender_user_id_) then
var = true
end
if msg.sender_user_id_ == tonumber(Kp_Owner) then
var = true
...
local var = false
if redis:sismember(RAMBOv2.."Bot:KpSudos", user_id) then
var = true
end
if user_id == tonumber(Kp_Owner) then
var = true
...
local var = true
if redis:get(RAMBOv2.."Kpjoin1") then
local channel = ''..redis:get(RAMBOv2..'Kpch1')..''
local url , res = https.request('https://api.telegram.org/bot'..RAMBOv2_TOKEN..'/getchatmember?chat_id='..channel..'&user_id='..msg.sender_user_id_)
...
local user_id
local user = false
local list1 = redis:smembers(RAMBOv2.."Bot:KpSudos")
...
redis:incr(RAMBOv2..'kpaddcon'..msg.chat_id_..':'..msg.sender_user_id_)
...
redis:set(RAMBOv2.."KpSudos" .. user, "> Sudo User ID : " .. sudouserid .. [[
...
redis:set(RAMBOv2.."KpOwnerBot" .. Kp_Owner, "> Bot Owner ID : " .. botowneruserid .. [[
...
redis:set(RAMBOv2..'rambonams',NAME_BOT)
...
-----------ch join------------------------------------------------------
if redis:get(RAMBOv2..'Kpch'..msg.sender_user_id_) then
redis:del(RAMBOv2..'Kpch'..msg.sender_user_id_)
...
return false 
elseif data.description == "Bad Request: chat not found" then -- التحقق من المعرف (@nnnnh)
...
redis:set(RAMBOv2..'Kpch1',CH_BOT)
...
-------------------------------leave groups----------------------------------------------------------------------
if is_sudo(msg) and idf:match("-100(%d+)") and (text:match('^'..(redis:get(RAMBOv2..'rambonams') or 'العراق')..' غادر$')) then
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmrambov2(msg).."\n🎟┊ تم تفعيل خاصية رفع الادمن\n ✓ ", 1, 'md')
...
redis:srem(RAMBOv2..hash, ap[2])
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserRambov2..'])\n⚠️┊ تم تنزيله من المطورين \n✓‏', 1, 'md')
table.remove(_config.Sudo_Users, getindex(_config.Sudo_Users, rami))
save_on_config()
load_config()
end
end
...
local UserRambov2 = user_info_
if user_info_ then
redis:set(RAMBOv2..'rr1'..msg.sender_user_id_..''..msg.chat_id_..'','rr')
send(msg.chat_id_, 0, 1, '👨‍✈️» اهلا ['..UserRambov2..'] \n™️» في لعبه السرعه\n⚠️» ارسل ( بدء اللعبه ) للعب\n✓',1, 'md')
return false end end
if text == 'بدء اللعبه'  and is_momod(msg.sender_user_id_, msg.chat_id_) and redis:get(RAMBOv2..'rr1'..msg.sender_user_id_..''..msg.chat_id_..'') then
local ramb1 = {'س م ى و','ى-س-م-و'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..ramb1[math.random(#ramb1)]..')*',1, 'md')
redis:set(RAMBOv2..'rr11'..msg.chat_id_..'','rrr')
return false end
if text then
local rambov21 = redis:get(RAMBOv2..'rr11'..msg.chat_id_..'')
if rambov21 == 'rrr' then
if text == 'موسى' then
redis:incr(RAMBOv2..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(RAMBOv2..'rr11'..msg.chat_id_..'')
sleep(1.5)
local ooo = {'ا-ل-ي-ي-ب','ي ي ا ل ب'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..ooo[math.random(#ooo)]..')*',1, 'md')
redis:set(RAMBOv2..'rr111'..msg.chat_id_..'','rrrr')
 return false end end end
if text then
local rambov21 = redis:get(RAMBOv2..'rr111'..msg.chat_id_..'')
if rambov21 == 'rrrr' then
if text == 'ليبيا' then
redis:incr(RAMBOv2..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(RAMBOv2..'rr111'..msg.chat_id_..'')
sleep(1.5) 
local ramb = {'ر ط ا ي ه','ا-ي-ط-ر-ه'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..ramb[math.random(#ramb)]..')*',1, 'md')
redis:set(RAMBOv2..'rr1111'..msg.chat_id_..'','mm')
 return false end end end
if text then
local rambov21 = redis:get(RAMBOv2..'rr1111'..msg.chat_id_..'')
if rambov21 == 'mm' then
if text == 'طياره' then
redis:incr(RAMBOv2..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(RAMBOv2..'rr1111'..msg.chat_id_..'')
sleep(1.5)
local ramb77 = {'😛😛😛😝😛😛😛','😜😝😜😜😜😜😜'}
send(msg.chat_id_, 0, 1, '•ارسل الاسمايل المختلف \n*('..ramb77[math.random(#ramb77)]..')*',1, 'md')
...
sleep(1.5)
local ramb2 = {'ش-ر-ط-ي','ش,ط,ر,ي'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..ramb2[math.random(#ramb2)]..')*',1, 'md')
...
sleep(1.5)
local ramb4 = {'ر ك و ا ي','ر ا ي ك و'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..ramb4[math.random(#ramb4)]..')*',1, 'md')
...
sleep(1.5)
local ramb5 = {'ك - م - ا - ل','ل ك ا م'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..ramb5[math.random(#ramb5)]..')*',1, 'md')
...
sleep(1.5)
local ramb6 = {'ل-ج-م-ي','ل ي م ج'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..ramb6[math.random(#ramb6)]..')*',1, 'md')
...
sleep(1.5)
local ramb7 = {'و ك ه ه','ه ك ه و'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..ramb7[math.random(#ramb7)]..')*',1, 'md')
...
sleep(1.5)
local ramb8 = {'ض ر م ي','م ض ر ي'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..ramb8[math.random(#ramb8)]..')*',1, 'md')
...
sleep(1.5)
local ramb9 = {'ي- ط -ر -ق -ن -ب -ا',' ب ي ق ا ط ر ا ن','ب,ي,ا,,ق,ر,ن,ط'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..ramb9[math.random(#ramb9)]..')*',1, 'md')
...
sleep(1.5)
local ramb00 = {'😔😔😔😔😔😔😔☺️😔😔😔😔😔'}
send(msg.chat_id_, 0, 1, '•ارسل الاسمايل المختلف \n*('..ramb00[math.random(#ramb00)]..')*',1, 'md')
...
sleep(1.5)
local rambp = {'ط - م - ي','ي ,م ,ط'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..rambp[math.random(#rambp)]..')*',1, 'md')
...
local UserRambov2 = user_info_
if user_info_ then
local numram = tonumber(redis:get(RAMBOv2..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'') or 0 )
if numram == 0 then
send(msg.chat_id_, msg.id_, 1, '• انت لا تمتلك نقاط',1, 'md')
else
local ramb = [[
• اهلا ~ []]..UserRambov2..[[]
®️┊نقاطك *(]]..numram..[[)* اذا اردت تحويلهم
💬┊لعضو مميز ارسل رقـــــم ( *1* ) 
 ✓
]]
redis:set(RAMBOv2..'rami1'..msg.sender_user_id_..''..msg.chat_id_..'','rami2')
send(msg.chat_id_, msg.id_, 1, ram,1, 'md')
return false end end end
if text then
local rambov21 = redis:get(RAMBOv2..'rami1'..msg.sender_user_id_..''..msg.chat_id_..'')
if rambov21 == 'rami2' then
if text == '1' then
...
redis:del(RAMBOv2..'rami1'..msg.sender_user_id_..''..msg.chat_id_..'')
else
local numram = tonumber(redis:get(RAMBOv2..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'') or 0 )
...
redis:del(RAMBOv2..'rami1'..msg.sender_user_id_..''..msg.chat_id_..'')
return false end
if numram > tonumber(redis:get(RAMBOv2.."RAMBOv2_OO0" .. msg.chat_id_) or 100 )  then
...
redis:setex(RAMBOv2..'Kpch'..msg.sender_user_id_,300,true)
...
if not redis:get(RAMBOv2..'Kpch1') then
send(msg.chat_id_, msg.id_, 1, '™️┊ لم يتم وضع القناة\n📌┊ لتعين  القناة ارسل .....\n⚠️┊ `تعين قناة الاشتراك `\n➖', 1, 'md')
return false 
end
if redis:get(RAMBOv2..'Kpch1') then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmrambov2(msg).."\n🎟┊ تم تفعيل الاشتراك الاجباري\n ✓ ", 1, 'md')
redis:set(RAMBOv2.."Kpjoin1", true)
return false end end end
...
send(msg.chat_id_, msg.id_, 1, ramb, 1, 'md')
else
rambov2 = [[

🙇🏿‍♂| مرحبا عزيزي انا بوت اسمي *(]]..(redis:get(RAMBOv2..'rambonams') or 'العراق')..[[)👮‍♂*
✅| اقوم بحماية مجموعتــك  مـن '
📊| الروابط، والتكرار، والتوجيه وغيرها '
⚜| قم بأضافة  البوت الى المجموعه ،
♻️| ثم ارسل (تفعيل) او راسل المطور،
🚸| لكي يتم تفعيله فْـي المجموعـِْـْه'
...
local start = redis:get(RAMBOv2.."startbot")
if start then
send(msg.chat_id_, msg.id_, 1, '*« هي الكليشه عزيزي »👇🏿*\n\n['..start..']', 1, 'md')
else
...
local Kpch2 = redis:get(RAMBOv2.."Kpch1")
if Kpch2 then
...
for rrrrk in string.gmatch(result.first_name_, "[^%s]+") do
result.first_name_ = rrrrk
break
end end end
...
send(v,0, 1,"- *تم تعطيل مجموعه* »\nﮧ┉┉┉┉┉┉┉┉┉\n‏🔱┊ المدير ≈ " .. owner .. "\n🔰┊ الرابط ≈ [اضغـط هنـا](" .. (redis:get(RAMBOv2.."bot:group:link"..msg.chat_id_) or "https://t.me/BECIQ") .. ")\n🔅┊* اسم المجموعه* »\n👨🏼┊ﮧ "..title_name(msg.chat_id_).."\n🚫┊ *ايدي المجموعه* »\n📉┊ﮧ "..msg.chat_id_.."\n*«معلومات عن المطور»*\nﮧ┉┉┉┉┉┉┉┉┉\n🌀┊ ايديه ≈ (" .. msg.sender_user_id_ .. ")\n🚫┊ اسمه ≈ " .. fname .. " " .. lname .. "\n️⚠️┊ معرفه ≈ [" .. username .. "]\n‏", 1, "md")
end
...
local num_ramb = (tonumber(redis:get(RAMBOv2.."incr_msg"..result.id_..""..msg.chat_id_.."") or 0 ))
local msgs = (tonumber(redis:get(RAMBOv2.."msgs:"..result.id_..":"..msg.chat_id_) or  0))
local Kpcontact = (tonumber(redis:get(RAMBOv2.."kpaddcon"..msg.chat_id_..":"..result.id_) or 0))
if result.id_ then
...
syria_info2 = "🎟┊ ايديه  » `" .. result.id_ .. "`\n©️┊ معرفه »  [@" .. ap[2] .. "]\n👤┊ جهاته  »   "..Kpcontact.."\n🔆┊ نقاطه  »   "..num_ramb.."\n🌐┊ تفاعله »  " .. KP_TM_NM(msgs) .. "\n✉️┊ رسائله » " .. msgs .. "\n📌┊ موقعه » " .. t .. "\n‏┄┄┄┄┄┄┄┄┄┄┄┄"
elseif not result.id_ then
syria_info2 = "🌀┊ لا يوجد عضو بهذا المعرف"
end
send(msg.chat_id_, msg.id_, 1, syria_info2, 1, "md")
end
...
if not redis:get(RAMBOv2..'lock:add'..msg.chat_id_) then
local num_ramb = tonumber(redis:get(RAMBOv2..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'') or 0 )
local msgs = tonumber(redis:get(RAMBOv2.."msgs:"..msg.sender_user_id_..":"..msg.chat_id_))
local Kpcontact = (tonumber(redis:get(RAMBOv2.."kpaddcon"..msg.chat_id_..":"..msg.sender_user_id_) or 0))
...
local syria_info = "🎟┊ ايديك  » " .. msg.sender_user_id_ .. "\n🖲┊ معرفك »  " .. username .. "\n👤┊ جهاتك  »   "..Kpcontact.."\n🌄┊ صورك  »   "..result.total_count_.."\n🌀┊ نقاطك  »   "..num_ramb.."\n🏌‍♂┊ تفاعلك »  " .. KP_TM_NM(msgs) .. "\n✉️┊ رسائلك » " .. msgs .. "\n📌┊ موقعك » " .. tar .. "\n‏┄┄┄┄┄┄┄┄┄┄┄┄"
local syria_info2 = "🎟┊ ايديك  » " .. msg.sender_user_id_ .. "\n🖲┊ معرفك »  [" .. username .. "]\n👤┊ جهاتك  »   "..Kpcontact.."\n🌄┊ صورك  »   "..result.total_count_.."\n🌀┊ نقاطك  »   "..num_ramb.."\n🏌‍♂┊ تفاعلك »  " .. KP_TM_NM(msgs) .. "\n✉️┊ رسائلك » " .. msgs .. "\n📌┊ موقعك » " .. tar .. "\n‏┄┄┄┄┄┄┄┄┄┄┄┄"

if redis:get(RAMBOv2.."getidstatus" .. msg.chat_id_) == "Photo" then
if result.photos_[0] then
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, result.photos_[0].sizes_[1].photo_.persistent_id_, syria_info)
else
...
local botnamess = redis:get(RAMBOv2.."rambonams") or "العراق"
...
local botnamess = redis:get(RAMBOv2.."rambonams") or "العراق"
local kpSudos1 = redis:scard(RAMBOv2.."Bot:KpSudos") or "0"
local kpAdmins = redis:scard(RAMBOv2.."Bot:Admins") or "0"
local kpgban = redis:scard(RAMBOv2.."bot:gban:") or "0"
local kpvipmems = redis:scard(RAMBOv2.."bot:vipmems:") or "0"
local kpgps = redis:scard(RAMBOv2.."bot:groups") or "0"
local kpusers = redis:scard(RAMBOv2.."bot:userss") or "0"
send(msg.chat_id_, msg.id_, 1, "*- احصائيات البـــــوت ≈*\n*ﮧ -------------------»*\n👨🏼┊اسم البوت ≈ *" .. botnamess .. "*\n🃏┊عدد المطورين ≈ *"..kpSudos1.."*\n🔰┊عدد الادمنيه ≈ *"..kpAdmins.."*\n🔚┊عدد الكروبات ≈ * "..kpgps.."*\n👪┊عدد المشتركين خاص ≈ *" .. kpusers .. "*\n🔔┊عدد المحظورين عام ≈ *"..kpgban.."*\n🔘┊عدد المميزين عام ≈ * "..kpvipmems.."*\n🗯┊قناة السورس ≈ [RAMBOv2](t.me/BECIQ)\n✓", 1,"md")
end
end
------------------GPS AND USERS-------------------------------------------
...
send(msg.chat_id_, msg.id_, 1, "🚫┊ الرتبه » (*"..rambov2.."*) 🍃\n‏" , 1, "md")
end
...
------------------SOURCE RAMBOv2---------------------------------------------------
if text:match("^سورس$") or text:match("^السورس$") or text:match("^مطور السورس$") or text:match("^ياسورس$") or  text:match("^سورس العراق$") or text:match("^اريد سورس$") then
if not redis:get(RAMBOv2..'lock:add'..msg.chat_id_) then
local text =  [[
‏
...
local kpbanned = redis:scard(RAMBOv2.."bot:banned:" .. msg.chat_id_) or "0"
...
local kpmomod = redis:scard(RAMBOv2.."bot:momod:" .. msg.chat_id_) or "0"
...
local kpowners = redis:scard(RAMBOv2.."bot:owners:" .. msg.chat_id_) or "0"
...
local kpmonshis = redis:scard(RAMBOv2.."bot:monshis:" .. msg.chat_id_) or "0"
...
local kpvipmem = redis:scard(RAMBOv2.."bot:vipmem:" .. msg.chat_id_) or "0"
...
local kpkeed = redis:scard(RAMBOv2.."bot:keed:" .. msg.chat_id_) or "0"
...
local kpmuted = redis:scard(RAMBOv2.."bot:muted:" .. msg.chat_id_) or "0"
...
os.execute('wget https://raw.githubusercontent.com/najdatmlqe/iraq/master/IRAQ.lua') 
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmrambov2(msg).."\n🎟┊ تم تحديث السورس \n ✓ ", 1, 'md')
dofile('IRAQ.lua')  
...
if not redis:get(RAMBOv2..'lock:add'..msg.chat_id_) then
local tmramb =  text:match("^اذاعه خاص (.*)")
local tmramb2 = "🌀┊ تم ارسال الاذاعه الى:\n( * GP * ) من الاعضاء\n‏"
local gp = tonumber(redis:scard(RAMBOv2.."bot:userss"))
gps = redis:smembers(RAMBOv2.."bot:userss")
text = tmramb2:gsub('GP',gp)
for k,v in pairs(gps) do
send(v, 0, 1,tmramb, 1, 'md')
end
send(msg.chat_id_, msg.id_, 1,text, 1, 'md')
end end end
----------------promote_admin------------------
...
local kpmonshis = redis:scard(RAMBOv2.."bot:monshis:" .. chattid) or "0"
local kpbanned = redis:scard(RAMBOv2.."bot:banned:" .. chattid) or "0"
local kpowners = redis:scard(RAMBOv2.."bot:owners:" .. chattid) or "0"
local kpmuted = redis:scard(RAMBOv2.."bot:muted:" .. chattid) or "0"
local kpkeed = redis:scard(RAMBOv2.."bot:keed:" .. chattid) or "0"
local kpmomod = redis:scard(RAMBOv2.."bot:momod:" .. chattid) or "0"
local kpvipmem = redis:scard(RAMBOv2.."bot:vipmem:" .. chattid) or "0"
...
if not redis:get(RAMBOv2..'lock:add'..msg.chat_id_) then
tdcli.sendDocument(RAMBOv2_SUDO, 0, 0, 1, nil, './IRAQ.lua', dl_cb, nil)
...
local Kpkick = redis:get(RAMBOv2.."kick_me"..msg.sender_user_id_..""..msg.chat_id_.."")
...
local Kpunkick = redis:get(RAMBOv2.."unkick_me"..msg.sender_user_id_..""..msg.chat_id_.."")
...
local kptext = (tonumber(redis:get(RAMBOv2.."text:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local kpsticker = (tonumber(redis:get(RAMBOv2.."sticker:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local pkPhoto = (tonumber(redis:get(RAMBOv2.."Photo:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local kpVoice = (tonumber(redis:get(RAMBOv2.."Voice:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local kpGif = (tonumber(redis:get(RAMBOv2.."Gif:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local kpVideo = (tonumber(redis:get(RAMBOv2.."Video:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local kpSelfVideo = (tonumber(redis:get(RAMBOv2.."SelfVideo:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local kpcon = (tonumber(redis:get(RAMBOv2.."kpaddcon"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
if result.first_name_ then
if #result.first_name_ < 25 then
else
for rrrrk in string.gmatch(result.first_name_, "[^%s]+") do
...
--------------------------------------------------------
if text == ''..(redis:get(RAMBOv2..'rambonams') or 'العراق')..' هينه' or text == ''..(redis:get(RAMBOv2..'rambonams') or 'العراق')..' رزله' then
...
--------------------------------------------------------------------------------------------
if text == ''..(redis:get(RAMBOv2..'rambonams') or 'العراق')..' بوسه' or text == ''..(redis:get(RAMBOv2..'rambonams') or 'العراق')..' مصه' then
function reep22(extra, result, success)
if tonumber(result.sender_user_id_) == tonumber(RAMBOv2_SUDO) then
...
if not redis:get(RAMBOv2..'lock:add'..msg.chat_id_) then
local RAMBOv2 = {'اسمي '..(redis:get(RAMBOv2..'rambonams') or 'العراق')..' 😌👌','تره اسمي '..(redis:get(RAMBOv2..'rambonams') or 'العراق')..' يالجريذي ☹️😹'  }
send(msg.chat_id_, msg.id_, 1,''..RAMBOv2[math.random(#RAMBOv2)]..'', 1, 'md')
end
end
-----------------------------------------
if text == ''..(redis:get(RAMBOv2..'rambonams') or 'العراق')..'' then
if not redis:get(RAMBOv2..'lock:add'..msg.chat_id_) then
local RAMBOv2 = {'اي نعم اني '..(redis:get(RAMBOv2..'rambonams') or 'العراق')..' 🍃😐','اهو اجوي الملطلطين 😹😪','كافي تره كرهت اسمي 💔😠'}
send(msg.chat_id_, msg.id_, 1,''..RAMBOv2[math.random(#RAMBOv2)]..'', 1, 'md')
end
end
-------------------------------------------------
...
redis:set(RAMBOv2.."Bot:KpBotAccount", data.value_.value_)
end end end end  end
----- End Source By rami RAMBO »»»»»»»»»»»»»»»»»»»